      ...
      <el-scrollbar height="100%" class="form-container">
        ...
        >
          ...
          <el-form-item label="切分字段" v-if="ruleForm.type === 0">
            <el-input
              v-model="ruleForm.splitPk"
              style="width: 300px"
              placeholder="多通道执行时用来切分的字段，一般使用主键"
            />
          </el-form-item>
          <el-form-item label="字段" v-if="ruleForm.type === 0">
            <el-checkbox v-model="ruleForm.checkAll" @change="onCheckAllChange"
              >全选
            </el-checkbox>
            <div style="margin: 15px 0" />
            <el-checkbox-group
              v-model="ruleForm.columns"
              v-if="!ruleForm.checkAll"
            >
              <el-checkbox
                v-for="columnName in ruleForm.columnList"
                :key="columnName"
                :value="columnName"
                >{{ columnName }}
              </el-checkbox>
...
const { reader: ruleForm, repoList, repoConfigList } = toRefs(props)
