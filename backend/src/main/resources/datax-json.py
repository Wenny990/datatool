...
def jsonToLocalFile(jsonFile,uuid):
    # 每天创建文件夹，以当前日期为文件夹名称，文件名为:job_时间戳.json
    try:
        jsonConfig = json.loads(jsonFile)
    except Exception:
        uuid = "%s_%s" % ('NOT_JOB_ID', int(time.time() * 1000))
    # uTableName = explainJson["job"]["content"][0]["writer"]["parameter"]["connection"][0]["table"][0]
    # tableName = uTableName.encode('utf-8')
    nowTime = time.strftime('%Y%m%d', time.localtime(time.time()))
    jsonPath = "%s/job/%s/" % (DATAX_HOME, nowTime)
    jsonName = "%s.json" % (uuid)
    print jsonPath
    print jsonName

    isExists = os.path.exists(jsonPath)
    if not isExists:
        try:
...
def getOptionParser():
    ...
    prodEnvOptionGroup.add_option("--uid", metavar="<job unique uuid>", dest="uid", action="store", default="", help="Set the log file name.")
    prodEnvOptionGroup.add_option("--json", metavar="<json string>", dest="json", action="store",
                                  help="Pass job config as JSON string. Will save to a temp file and run.")
...
def buildStartCommand(options, args):
    ...
    if options.uid:
        logFileName = options.uid
    else:
        logFileName = jobResource[-20:].replace('/', '_').replace('.', '_')

    jobParams = ("-Dlog.file.name=%s") % (logFileName)
    if options.params:
...
if __name__ == "__main__":
    ...
    # 新增：处理 --json
    if options.json:
        args = [jsonToLocalFile(options.json,options.uid)]
        # 返回绝对路径给上游
        print("Job saved success!!!")
